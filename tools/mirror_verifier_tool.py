...
_TAG_STYLE_FALLBACKS = {
    "success": {"foreground": "#0f5132"},
    "danger": {"foreground": "#842029"},
    "warning": {"foreground": "#664d03"},
    "secondary": {"foreground": "#41464b"},
}
        ...
        for status, style in _STATUS_TAGS.items():
            if style:
                try:
                    ...
                    fallback = _TAG_STYLE_FALLBACKS.get(style)
                    if fallback:
                        self.results_tv.tag_configure(status, **fallback)
